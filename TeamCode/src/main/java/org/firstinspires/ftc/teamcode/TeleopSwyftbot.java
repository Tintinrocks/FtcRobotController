 ...
 */
@TeleOp(name="Teleop-Swyftbot", group="7592")
//@Disabled
public class TeleopSwyftbot extends LinearOpMode {
...
//      gamepad2_share_last      = gamepad2_share_now;       gamepad2_share_now      = gamepad2.share;
    ...
    /*---------------------------------------------------------------------------------*/
    void resetInjector() {
        // Make sure the injector is reset before we allow any spindexer rotation!
        if (injectorReset == false) {
            robot.liftServo.setPosition(robot.LIFT_SERVO_RESET);
            sleep(100);
            injectorReset = true;
        }
    } // resetInjector
    ...
    void processInjector() {
        // Check for an OFF-to-ON toggle of the gamepad2 TRIANGLE button (toggles INJECTOR up/down)
        if( gamepad2_triangle_now && !gamepad2_triangle_last)
        {
            if (injectorReset == false){
                ...
                robot.liftServo.setPosition( robot.LIFT_SERVO_INJECT );
                injectorReset = false;
            }
        }
    } // processInjector
